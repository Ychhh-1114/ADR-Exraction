...
class Trainer:
    ...
    def bert_predict(self):
        ...
        for i, data_item in pbar:
            ...
            logits[0][0] = 0
            _, pred_rel = torch.max(logits.data, 1)
            rel_pred.append(self.id2rel[int(pred_rel)])
        # self.model.train(True)
        # # rel_pred = [[] for _ in range(len(pred_rel))]
        #
        #
        ...
        return  rel_pred #rel_pred


if __name__ == '__main__':
    print("Run EntityRelationExtraction REL BERT ...")
    config = ConfigRel()
    model = BertForSequenceClassification.from_pretrained('../data_loader/biobert-base-cased-v1.2',
                                                          num_labels=config.num_relations)
